...
def main(node: Node):
    ...
    # Filter by end index
    if node.end_index:
        print(f"Getting stations until idx {node.end_index} ...")
        networks = networks[:node.end_index]
        stations = stations[:node.end_index]
        latitudes = latitudes[:node.end_index]
        longitudes = longitudes[:node.end_index]
        burials = burials[:node.end_index]

    # Filter by start index
    if node.start_index:
        print(f"Getting stations from idx {node.start_index} ...")
        networks = networks[node.start_index:]
        stations = stations[node.start_index:]
        latitudes = latitudes[node.start_index:]
        longitudes = longitudes[node.start_index:]
        burials = burials[node.start_index:]

    # Add stations
    node.add(run_stations, networks=networks, stations=stations,
             latitudes=latitudes, longitudes=longitudes, burials=burials)

    # Run the forward test once the stations are done
    if node.forward_test == True:
        node.add(forward_test, networks=networks, stations=stations,
                 latitudes=latitudes, longitudes=longitudes, burials=burials)


def run_stations(node: Node):
    node.concurrent = True

    # Add station configuration
    for net, sta, lat, lon, bur in zip(
        node.networks, node.stations,
        node.latitudes, node.longitudes, node.burials):
